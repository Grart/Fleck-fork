	...
	public static class Draft76Handler
    {
        ...
        public static void ReceiveData(Action<string> onMessage, List<byte> data)
        {
            while (data.Count > 0)
            {
				...
				var bytes = LinqExtensions.SkipTakeToArray(
								data.ToArray(),
								1,
								endIndex - 1
							);
        ...
        private static byte[] ParseKey(string key)
        {
			...
			int spaces = LinqExtensions.CountSpaceChar(key);

			//var digits = new String(key.Where(Char.IsDigit).ToArray());
			var digits = new String(LinqExtensions.WhereToArray(key, Char.IsDigit));
