...
namespace Fleck
{
	public class WebSocketConnection : IWebSocketConnection
	{
		public WebSocketConnection(
				...
				Func<string[], string> negotiateSubProtocol
			)
		{
			Socket = socket;
			OnOpen = () => { };
		...
		private readonly Func<string[], string> _negotiateSubProtocol;
		...
		{
			Send(message, Handler.FrameText);//return
		}

		//public Task Send
		public void Send(byte[] message)
		{
			Send(message, Handler.FrameBinary);//return
		}

		//public Task SendPing
		public void SendPing(byte[] message)
		{
			Send(message, Handler.FramePing);//return
		}

		//public Task SendPong
		public void SendPong(byte[] message)
		{
			Send(message, Handler.FramePong);//return
		}

		//public Task Send<T>
		private void Send<T>(
				T message, 
				Func<T, byte[]> createFrame
			)
		{
			if (Handler == null)
		...
		public void CreateHandler(List<byte> data)
		{
		...
		private void Read(List<byte> data, byte[] buffer)
		{
			...
			Socket.Receive(
				...
				r =>
				{
					...
					var readBytes = LinqExtensions.SkipTakeToArray(buffer, 0, r);
					if (Handler != null)
					{
