...
namespace Fleck
{
	public class SocketWrapper : ISocket
	{
			...
			)
		{
			...
			try
			{
				_stream.BeginRead(
					...
					offset,
					buffer.Length,
					SocketReadAsyncCallback,
					new SocketAsyncState<int, Stream>()
					{
						_Callback = callback,
						_Error = error,
						_CancelSource = _tokenSource,
						_SocketOrStream = _stream
					}
				);
			}
			catch(Exception ex)
		...
		private static void SocketReadAsyncCallback(
				IAsyncResult asynResult
			)
		{
			var _state = asynResult.AsyncState as SocketAsyncState<int, Stream>;
			try
			{
				...
				{
					var _ret = _state._SocketOrStream.EndRead(asynResult);
					_state._Callback(_ret);
				}
			}
			catch (Exception ex)
			{
			...
			)
		{
			...
			{
				_socket.BeginAccept(
					SocketAcceptAsyncCallback,
					new SocketAsyncState<ISocket, Socket>()
					{
						_Callback = callback,
						_Error = error,
						_CancelSource= _tokenSource,
						_SocketOrStream = _socket
					}
				);
			}
			catch (Exception ex)
		...
		private static void SocketWriteAsyncCallback(
				IAsyncResult asynResult
			)
		{
			var _state = asynResult.AsyncState as SocketAsyncState<int, Stream>;
			try
			{
				...
				{
					_state._SocketOrStream.EndWrite(asynResult);
					_state._Callback(0);
				}
			}
			catch (Exception ex)
	...
	class SocketAsyncState<T, TSocket> : IDisposable
		where TSocket : class
	{
		internal Action<T> _Callback;
		internal Action<Exception> _Error;
		internal CancellationTokenSource _CancelSource;
		internal TSocket _SocketOrStream;
